...
function liquid_single_woo_scripts() {
	...
	if ( apply_filters( 'liquid_ajax_add_to_cart_single_product', '__return_true' ) ) {
		wp_enqueue_script( 'liquid-ajax-add-to-cart', get_template_directory_uri() . '/liquid/vendors/woocommerce/js/liquid-ajax-add-to-cart.js', array( 'jquery' ), null, true );
		wp_localize_script( 'liquid-ajax-add-to-cart', 'liquid_ajax_object', array( 'ajax_url' => admin_url( 'admin-ajax.php' ) ) );
	}
}
...
function liquid_wc_enqueue_scripts() {
	...
	wp_enqueue_script( 'liquid-ajax-filter', get_template_directory_uri() . '/liquid/vendors/woocommerce/js/liquid-ajax-filter.js', array( 'jquery' ), null, true );

	$options = [
		'wcHiddenSidebar'            => liquid_helper()->get_kit_option( 'liquid_wc_archive_show_product_cats' ),
		'wcAjaxFilter'               => liquid_helper()->get_kit_option( 'liquid_wc_ajax_filter' ),
		'wcAjaxPagination'           => liquid_helper()->get_kit_option( 'liquid_wc_ajax_pagination' ),
		'wcAjaxPaginationType'       => liquid_helper()->get_kit_option( 'liquid_wc_ajax_pagination_type' ),
		'wcAjaxPaginationButtonText' => liquid_helper()->get_kit_option( 'liquid_wc_ajax_pagination_button_text' ),
	];
	...
	wp_localize_script( 'liquid-ajax-filter', 'wcLiquid', [
...
function liquid_wc_body_class( $classes ) {
	if ( liquid_helper()->get_kit_option( 'liquid_wc_archive_show_product_cats' ) === 'on' || ( isset( $_GET['opt'] ) && $_GET['opt'] === 'wcHiddenSidebar' ) ) {
