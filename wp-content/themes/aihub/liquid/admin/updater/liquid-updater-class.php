...
class Liquid_Updater {
	...
	function __construct( $args = array(), $strings = array() ) {
		...
		$args = wp_parse_args( $args, array(
			'remote_api_url' => 'http://api.liquid-themes.com/aihub',
			'request_data' => array(),
	...
	function update_nag() {
		...
		if ( version_compare( $this->version, $api_response->new_version, '<' ) ) {
			...
			printf(
				wp_kses( __( '<strong>%1$s %2$s</strong> is available. <a href="%3$s" class="thickbox" title="%4s">Check out what\'s new</a> or <a href="%5$s"%6$s>update now</a>.', 'aihub' ), 'lqd_post' ),
				$theme->get( 'Name' ),
			...
			if ( !$failed ) {
				...
				if ( version_compare( $this->version, $update_data->stable_version, '<' ) ) {
					...
					$title = sprintf( 
						...
						__( 'Theme update available!', 'aihub' ), 
					);

					$content = sprintf(
						'<p>AIHub v%s %s</p><p>%s</p>',
						$update_data->stable_version,
						__( 'is available!', 'aihub' ),
